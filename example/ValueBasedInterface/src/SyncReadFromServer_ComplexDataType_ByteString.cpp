...
class ExampleClient
{
  public:
	...
	bool readFromServer(void)
	{
		...
		OpcUaExtensionObject::SPtr complexValue;
		complexValue = dataValue.variant()->variantSPtr<OpcUaExtensionObject>();

		if (complexValue->typeId() != OpcUaNodeId(3002, 1)) {
			std::cout << std::endl << "**** data value type error ****" << std::endl;
			return false;
		}

		OpcUaByteString::SPtr byteString;
		byteString = complexValue->byteString();
		std::cout << std::endl << "**** " << byteString->toString() << " ****" << std::endl;
